<template>
	<div class="body">
		...
		<div style="padding: 0px 20px">
			...
			<view class="illustrate">请在下方告诉AI你需要什么样的内容</view>
			<el-input class="textareaBox" @click="isinit" type="textarea" :rows="10" placeholder="请输入" v-model="item.body" />
			<button @click="generate" class="determineTheBuild" type="primary">生成</button>
			<button @click="router().back()" class="determineTheBuildBack" type="primary">返回创作中心</button>
		</div>
		</div>
	</div>
</template>
...
export default {
	...
	setup() {
        ...
        let isfirstEdit = ref(false);
		
        function isinit(){
		   if(!isfirstEdit.value){
			  isfirstEdit = ref(true); 
			  item.value.body="";
		   }	
		}
