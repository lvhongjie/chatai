<template>
  <div class="panel-container">
    <div class="body" v-if="mainPageVisible">
      <div class="article" v-if="store.getters.userinfo">会员套餐列表</div>
      <div class="introduce" v-if="store.getters.userinfo">
        magicAI一次消耗1个AI币，plusAi一次消耗30个AI币，绘画一次消耗20个AI币
      </div>
      <ViewState
        class="state"
...
import {alipayIsSucceed, alipayPayQrCode, GetProducts, GetUserInfo,wxpayPayQrCode} from "../../api/BSideApi";
  ...
  setup() {
    ...
    async function alipayPay() {
      try {
		...
		outcome.value = await wxpayPayQrCode(productId.value);
