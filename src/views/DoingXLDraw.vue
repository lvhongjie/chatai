<template>
 ...
 <el-dialog v-model="drawDialogVisible" class="el-dialog" style="  background-color: rgb(29, 32, 34);" align-center="true"
   ...
   <div class="iframe-container" id="myIframeContainer">
       <iframe ref="myIframe"  style="border: 0px;padding: 0px;" src="/draw/#/pages/xldraw/drawxl"></iframe>
     </div>
...
export default {
	...
	async function handleMySelftImages(pageNum) {
	  localStorage.setItem("xldrawjonbid",0);
	  ...
	  try {
		  load.value = true
		  let res = await MySelftImages(pageNum+"&type=2")
		   if (res.length) {
	 ...
	 function closeModal() {
	   ...
	   function getDrawinfo() {
		 setInterval(() => {
		   // 发送请求
		    let drawjonbid= localStorage.getItem("xldrawjonbid")
