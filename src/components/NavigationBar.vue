<template>
	<nav class="header">
		<div class="header-side">
			<div @click="appletDialogVisible = true" class="rechargeButton hidden-xs-only applet">
				<img class="appletIcon" :src="require('../assets/logo02.svg')">
				<div class="rechargeButtonText">MAGIC AI</div>
			</div>
		</div>
    ...
    <div v-if="store.getters.userinfo" class="header-side header-right">
      <div @click="router().push({
        ...
        <div class="rechargeButtonText">套餐</div>
      </div>
          ...
          <el-dropdown ref="dropdown1" trigger="contextmenu">
            <div @click="showClick" class="header-user-btn">
              ...
              <div class="header-user-name">{{
                  store.getters.userinfo.userName ? store.getters.userinfo.userName : '会员'
                }}
              </div>
            </div>
            <template #dropdown>
	...
	<el-dialog v-model="appletDialogVisible" title="MAGIC AI PLUS" width="300" center align-center>
...
.applet {
	width: 120px;
	margin-left: 10px;
	background-color: rgb(31,124,143);
}
